...
if uploaded_file is not None:
    ...
    with col_right:
        ...
        st.pyplot(fig2)
    # 4. WORD CLOUD
    with st.expander("☁️ View Issue Word Cloud", expanded=False):
        neg_text = " ".join(df[df['Predicted_Sentiment'] == 'Negative']['Comment_Text'].astype(str))
        if neg_text:
            wc = WordCloud(width=1200, height=400, background_color='white', colormap='Reds').generate(neg_text)
